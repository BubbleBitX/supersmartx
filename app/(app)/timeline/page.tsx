...
import { deleteFromTimeline, groupByYear, loadTimeline, TIMELINE_UPDATED_EVENT } from "@/lib/timeline";

export default function TimelinePage() {
  const [events, setEvents] = useState(loadTimeline);
    ...
    <div style={{ padding: "28px 32px", maxWidth: "980px" }}>
      ...
      <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "24px" }}>
        ...
        {EVENT_CATEGORIES.filter((category) => events.some((event) => event.category === category.slug)).map((category) => (
          <button key={category.slug} onClick={() => setFilterCategory(category.slug)} style={{
            padding: "4px 12px",
            fontSize: "11px",
            borderRadius: "20px",
            cursor: "pointer",
            background: filterCategory === category.slug ? category.color : "#161616",
            color: filterCategory === category.slug ? "#000" : "#666",
            border: `1px solid ${filterCategory === category.slug ? category.color : "#252525"}`,
            fontWeight: filterCategory === category.slug ? 700 : 400,
          }}>
